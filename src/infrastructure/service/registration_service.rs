...
use crate::{
    application::{
        ...
        error::AppError,
        repository::registration_repository::RegistrationRepository,
...
impl RegistrationService for RegistrationServiceImpl {
    ...
    async fn transition_status(
        ...
        let next = RegistrationStatus::try_from(dto.status.as_str()).map_err(|_| {
            AppError::Validation(format!("unknown status: {}", dto.status))
        })?;
